...
int CVode(void *cvode_mem, realtype tout, N_Vector yout,
   ...
   */
     ...
     */
        ...
        printf("Calling user supplied f function, ODE rhs for the first time\n");
        ...
        if (cv_mem->cv_sensi) {
            printf("Calling \"cvSensRhsWrapper\" for first time\n");
   ...
   */
    ...
    for (;;) {
        ...
        printf("calling cvStep to take a step internal to cvode\n");
        kflag = cvStep(cv_mem);
...
int cvSensRhsWrapper(CVodeMem cv_mem, realtype time,
    ...
    if (cv_mem->cv_ifS == CV_ALLSENS) {
        printf("Computing all sens at once, not one parameter at a time for time %f\n", time);
        retval = cv_mem->cv_fS(cv_mem->cv_Ns, time, ycur, fcur, yScur,
                               fScur, cv_mem->cv_fS_data, temp1, temp2);
        cv_mem->cv_nfSe++;
    } else {
        printf("Computing sens one parameter at for time %f\n", time);
        for (is = 0; is < cv_mem->cv_Ns; is++) {
...
int cvSensRhsInternalDQ(int Ns, realtype t,
    ...
    int is, retval;
    printf("calling: cvSensRhsInternalDQ\n");
    printf("Time t=%f\n", t);
    printf("N_vector y: \n");
    y->ops->nvprint(y);
    printf("N_vector ydot: \n");
    y->ops->nvprint(ydot);

    for (is = 0; is < Ns; is++) {
        printf("isth %d N_vector yS before computation: \n", is);
        y->ops->nvprint(yS[is]);
        printf("isth %d N_vector ySdot before computation: \n", is);
        y->ops->nvprint(ySdot[is]);
        retval = cvSensRhs1InternalDQ(Ns, t, y, ydot, is, yS[is],
                                      ySdot[is], cvode_mem, ytemp, ftemp);
        printf("isth %d N_vector yS after computation: \n", is);
        y->ops->nvprint(yS[is]);
        printf("isth %d N_vector ySdot after computation: \n", is);
        y->ops->nvprint(ySdot[is]);
        if (retval != 0) return (retval);
    }
...
int cvSensRhs1InternalDQ(int Ns, realtype t,
                         ...
                         void *cvode_mem,
                         N_Vector ytemp, N_Vector ftemp) {
    printf("inside: cvSensRhs1InternalDQ\n");
    CVodeMem cv_mem;
    int retval, method;
    int nfel = 0, which;
    ...
    Deltap = pbari * delta;
    rDeltap = ONE / Deltap;
    printf("which parameter is %d\n", which);
    printf("pbari is %f\n", pbari);
    printf("p is %f\n", cv_mem->cv_p[which]);
    printf("cv_mem->cv_ewt is: \n");
    cv_mem->cv_ewt->ops->nvprint(cv_mem->cv_ewt);
    ...
    switch (method) {
        ...
        case CENTERED1:
            ...
            printf("psave: %f, Delta: %f psave + Delta: %f \n", psave, Delta, psave + Delta);
            cv_mem->cv_p[which] = psave + Delta;
            printf("3 ysDot\n");
            ySdot->ops->nvprint(ySdot);

            printf("y before first call to cv_f: \n");
            ySdot->ops->nvprint(y);

            printf("ytemp: before first call to cv_f\n");
            ySdot->ops->nvprint(ytemp);

            retval = cv_mem->cv_f(t, ytemp, ySdot, cv_mem->cv_user_data);

            printf("y: after first call to cv_f \n");
            ySdot->ops->nvprint(y);

            printf("ytemp: after first call to cv_f \n");
            ySdot->ops->nvprint(ytemp);
    ...
    printf("end of \"cvSensRhs1InternalDQ\"\n");
