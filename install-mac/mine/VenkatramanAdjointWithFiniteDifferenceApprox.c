...
#define DTOUT RCONST(1)     /* output time increment         */
...
//    retval = CVodeWFtolerances(cvode_mem, ewt);
    ...
    retval = CVodeF(cvode_mem, TOUT, y, &t, CV_NORMAL, &ncheck);
    if(check_retval(&retval, "CVodeF", 1)) return(1);

    /* Allocate uB */
    N_Vector uB = N_VNew_Serial(NEQ);
    ...
    retval = CVodeSetUserDataB(cvode_mem, indexB, userData);
    ...
    retval = CVodeSStolerancesB(cvode_mem, indexB, RTOLB, ATOL);
