...
static void PrintVectorArray3x2(N_Vector *nVectorArray) {
    realtype *sdata;
    sdata = N_VGetArrayPointer(nVectorArray[0]);
    printf("\n\t\t\t S1 \t\t S2\n \t kin\t %f \t %f\n", sdata[0], sdata[1]);
    sdata = N_VGetArrayPointer(nVectorArray[1]);
    printf("\t kf\t\t %f \t %f\n", sdata[0], sdata[1]);
    sdata = N_VGetArrayPointer(nVectorArray[2]);
    printf("\t kout\t %f \t %f\n\t\n", sdata[0], sdata[1]);
...
//    CheckRetValNotNegative(&retval, "CVodeSetJacFn");
    ...
    if (with_sens) {
        ...
        printf("size uS[0] %d\n", uS[0]->ops->nvgetlength(uS[0]));
        printf("size uS[1] %d\n", uS[1]->ops->nvgetlength(uS[1]));
        printf("size uS[2] %d\n", uS[2]->ops->nvgetlength(uS[2]));

        // set sens init cond
        // So what is the sens init conditions?
        for (is = 0; is < NS; is++) {
            // iterate over num parameters
            double *data = uS[0]->ops->nvgetarraypointer(uS[is]);
//            data[0] = NV_Ith_S(y, 0);
//            data[1] = NV_Ith_S(y, 1);
            data[0] = 0.0;//NV_Ith_S(y, 0);
            data[1] = 0.0;//NV_Ith_S(y, 1);
//            NV_Ith_S(uS[is], y[is]);
        }

        printf("Initial uS:\n");
        PrintVectorArray3x2(uS);

        retval = CVodeSensInit1(cvode_mem, NS, sensi_meth, fS, uS);
