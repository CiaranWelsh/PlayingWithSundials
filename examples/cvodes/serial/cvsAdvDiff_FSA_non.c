...
//  ProcessArgs(argc, argv, &sensi, &sensi_meth, &err_con);
  ...
  if(sensi) {
    ...
    /**
     * Notice the lack of arrgument to fs1 - this means that
     * finite differences should be used instead.
     */
    ...
    printf("p[0]: %f; p[0]: %f; p[0]: %f; \n", data->p[0], data->p[1], data->p[2]);
    printf("pbar[0]: %f; pbar[0]: %f; pbar[0]: %f; \n", pbar[0], pbar[1], pbar[2]);
    printf("plist[0]: %d; plist[0]: %d; plist[0]: %d; \n", plist[0], plist[1], plist[2]);


    /****************************************************
     * Here data->p, pbar and plist with the absence of
     * an argument to fs in 'CVodeSensInit1' signal sundials
